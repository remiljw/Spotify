...
def show_playlist(self,request):
    desired_artist = self.request.GET("artist_to_search")
    ...
    if len(desired_artist_results["artists"]["items"]) == 0:
        return redirect('artist_not_found')
    ...
    sp.trace = False
    playlist_name = sp.user_playlist_create(username, playlist_name)
    ...
    return (request, 'playlist_maker/playlist.html', context)